...
#   3. GENERALIZAR las fechas que identifican casi en solitario a una persona:
#        - fecha_nac -> anyo_nac (solo el anyo)
#        - fecha_matricula -> anyo-mes
#   4. CONSERVAR sin tocar el resto -- incluidos cod_postal, nacionalidad,
#      pais_nac y municipio_nac (decision de negocio: se necesitan tal cual para
#      el analisis) y sexo, curso, grupo, turno, estado_matricula, notas...
#      El recorte a la lista blanca lo hace el Paso 4 (transformacion).
#
...
def anonimizar_tabla(entrada, salida, tabla, clave):
    ...
    print(f"{tabla}: {len(filas)} filas -> {salida}")
...
def procesar_curso(curso, clave):
    ...
    for tabla in TABLAS:
        ...
        if COLUMNAS_ID_ALUMNO[tabla] or COLUMNAS_FUERA[tabla] or GENERALIZAR[tabla]:
